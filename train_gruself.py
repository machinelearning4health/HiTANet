...
import random
import shutil
from models.untils import GRUSelfAttentionModel
...
def train_model(training_file='training_file',
    ...
    model = GRUSelfAttentionModel(n_diagnosis_codes, batch_size, options)
    ...
    for epoch in range(n_epoch):
        ...
        for index in samples:
            ...
            predictions, labels = model(batch_diagnosis_codes, batch_time_step, batch_labels, options, maxlen)
            optimizer.zero_grad()
        ...
        validate_cost = rnn_tools.calculate_cost_mine(model, validate, options, focal_loss)
        test_cost = rnn_tools.calculate_cost_mine(model, test, options, focal_loss)
...
if __name__ == '__main__':
    ...
    use_gpu = True
    model_name = 'gru_self_kidney_Relu'
    disease = 'kidney'
    print(model_name)
    results = []
    ...
    for disease in disease_list:
        ...
        total_results.append(np.mean(results, 0))
