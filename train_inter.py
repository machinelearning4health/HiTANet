...
import random
import shutil
from models.untils import Timeline
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, roc_auc_score
from torch.multiprocessing import Process
import traceback


def get_data(train, index, batch_size, max_len, n_diagnosis_codes):
    ...
    return [batch_diagnosis_codes, batch_time_step, batch_labels, lengths, maxlen]
...
def train_model(training_file='training_file',
    ...
    model = Timeline(batch_size, 256, 128, 64, n_diagnosis_codes, 2, dropout_rate)
    ...
    for epoch in range(n_epoch):
        ...
        for index in samples:
            ...
            t_diagnosis_codes, t_labels, t_mask, t_time, t_mask_final = rnn_tools.pad_matrix_time(batch_diagnosis_codes, batch_labels, batch_time_step, options)
            if use_gpu:
        ...
        validate_cost = rnn_tools.calculate_cost_time(model, validate, options, max_len, focal_loss)
        test_cost = rnn_tools.calculate_cost_time(model, test, options, max_len, focal_loss)
    ...
    for index in range(n_batches):
        ...
        maxlen = np.max(lengths)
        # [batch_diagnosis_codes, batch_time_step, batch_labels, lengths, maxlen] = training_queue.get(block=True)
        t_diagnosis_codes, t_labels, t_mask, t_time, t_mask_final = rnn_tools.pad_matrix_time(batch_diagnosis_codes, batch_labels,
...
if __name__ == '__main__':
    ...
    for disease in disease_list:
        model_name = 'inter_L4_wt_1e-4_focal%.2f' %(gamma)
        print(model_name)
