...
import time
import random
from models.TimeAware import TimeLSTM
import shutil
...
def train_model(training_file='training_file',
    ...
    model = TimeLSTM(n_diagnosis_codes, hidden_size, True, False)
    ...
    for index in range(n_batches):
        ...
        maxlen = np.max(lengths)
        # [batch_diagnosis_codes, batch_time_step, batch_labels, lengths, maxlen] = training_queue.get(block=True)
        t_diagnosis_codes, t_labels, t_mask, t_time, t_mask_final = rnn_tools.pad_matrix_retainEx(
            batch_diagnosis_codes, batch_labels,
...
if __name__ == '__main__':
    ...
    for disease in disease_list:
        model_name = 'timeaware_L4_wt_1e-4_focal%.2f' % (gamma)
        print(model_name)
        ...
        for k in range(5):
            accuary, precision, recall, f1, roc_auc = train_model(trianing_file, validation_file,
                                                                  ...
                                                                  use_gpu, model_name, disease=disease,
                                                                  code2id=code2id, gamma=gamma)
