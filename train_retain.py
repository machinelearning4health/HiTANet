...
def train_model(training_file = 'training_file',
    ...
    rnn = untils.RetainNN(options)
    if use_gpu:
        rnn = rnn.cuda()
    focal_loss = torch.nn.functional.cross_entropy
    rnn.train()
    print('constructing the optimizer ...')
    optimizer = torch.optim.Adam(rnn.parameters(), weight_decay = options['L2_reg'])
    print('done!')
...
if __name__ == '__main__':
    ...
    use_gpu = True
    model_name = 'retain'
    disease_list = ['copd', 'hf', 'kidney', 'amnesia', 'dementias']
    total_results = []
