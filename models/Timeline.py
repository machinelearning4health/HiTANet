...
def preprocessing(visitfile,labelfile,gapfile,batchsize):
    ...
    for start_ix in range(0, len(new_data)-batchsize+1, batchsize):
        ...
        for i in xrange(gap_matrix.shape[0]):
            mylabel.append(thisblock[i][1])
            for j in xrange(gap_matrix.shape[1]):
                try:
...
def train_model(batch_data,code_to_ix,label_to_ix, EMBEDDING_DIM, HIDDEN_DIM, ATTENTION_DIM, EPOCH, batchsize,dropoutrate):
...
def test_model(batch_data, model): 
        ...
        label_scores = model((inputs[0].cuda(),inputs[1].cuda(),inputs[2].cuda()), 0)
...
def parse_arguments(parser):
    ...
    parser.add_argument('--outputmodel', type=str, default='trainedmodel')
...
if __name__ == '__main__':
    ...
    trainedmodel=train_model(data,c2ix,l2ix,args.EMBEDDING_DIM,args.HIDDEN_DIM, args.ATTENTION_DIM,args.EPOCH, args.batchsize,args.dropoutrate)
    torch.save(trainedmodel.state_dict(), args.outputmodel)     
