...
import torch
from models.untils import adjust_input
...
def pad_matrix_time(seq_diagnosis_codes, seq_labels, seq_times, options):
    lengths = np.array([len(seq) for seq in seq_diagnosis_codes])
    lengths_code = []
...
def pad_matrix_retainEx(seq_diagnosis_codes, seq_labels, seq_times, options):
    lengths = np.array([len(seq) for seq in seq_diagnosis_codes])
    lengths_code = []
    for seq in seq_diagnosis_codes:
        for code_set in seq:
...
def pad_matrix_mine(seq_diagnosis_codes, seq_labels, options):
    lengths = np.array([len(seq) for seq in seq_diagnosis_codes])
    n_samples = len(seq_diagnosis_codes)
    n_diagnosis_codes = options['n_diagnosis_codes']
    maxlen = np.max(lengths)
    ...
    return batch_diagnosis_codes, batch_labels, batch_mask, batch_mask_final
    ...
    for index in range(n_batches):
        ...
        for ind in range(len(batch_diagnosis_codes)):
            if len(batch_diagnosis_codes[ind]) > 50:
                batch_diagnosis_codes[ind] = batch_diagnosis_codes[ind][-50:]
        batch_labels = data[1][batch_size * index: batch_size * (index + 1)]
        t_diagnosis_codes, t_labels, t_mask, t_mask_final = pad_matrix_mine(batch_diagnosis_codes, batch_labels, options)
    ...
    for index in range(n_batches):
        ...
        for ind in range(len(batch_diagnosis_codes)):
            if len(batch_diagnosis_codes[ind]) > 50:
                batch_diagnosis_codes[ind] = batch_diagnosis_codes[ind][-50:]
                batch_time_step[ind] = batch_time_step[ind][-50:]
        t_diagnosis_codes, t_labels, t_mask, t_time, t_mask_final = pad_matrix_time(batch_diagnosis_codes, batch_labels, batch_time_step,
                                                                            options)
    ...
    for index in range(n_batches):
        ...
        for ind in range(len(batch_diagnosis_codes)):
            if len(batch_diagnosis_codes[ind]) > 50:
                batch_diagnosis_codes[ind] = batch_diagnosis_codes[ind][-50:]
                batch_time_step[ind] = batch_time_step[ind][-50:]
        t_diagnosis_codes, t_labels, t_mask, t_time, t_mask_final = pad_matrix_retainEx(batch_diagnosis_codes, batch_labels, batch_time_step,
                                                                            options)
